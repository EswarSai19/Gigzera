...
def index(request):
    ...
    context = {'jobs': jobs}
    return render(request, 'non_register_mb/index.html', context)
...
def jobs(request):
    ...
    return render(request, 'non_register_mb/jobs.html', context)

def aboutus(request):
    return render(request, 'non_register_mb/aboutus.html')

def industries(request):
    return render(request, 'non_register_mb/industries.html')

def findajob(request):
    return render(request, 'non_register_mb/findajob.html')

def postajob(request):
    return render(request, 'non_register_mb/postajob.html')

def signup(request):
    return render(request, 'non_register_mb/signup.html')
...
def login(request):
    if request.method == 'POST':
        ...
        if freelancer and check_password(password, freelancer.password):
            ...
            return redirect('fl_index_mb')  # Redirect to freelancer dashboard
            ...
            return redirect('cl_index_mb')  # Redirect to client dashboard

        else:
            messages.error(request, 'Invalid credentials')

    return render(request, 'non_register_mb/login.html')


def forgot(request):
    return render(request, 'non_register_mb/forgot.html')
...
def test(request):
    ...
    for job in jobs:
        job.skills_list = [skill.strip().title() for skill in job.skills_required.split(',')]
    context = {'jobs': jobs}
    return render(request, 'non_register_mb/test.html', context)
...
def submit_contact(request):
    if request.method == 'POST':
        ...
        return redirect('index_mb')  # Redirect to home page
    ...
    if request.method == "POST":
        ...
        if password1 == password2:
            if Freelancer.objects.filter(email=email).exists():
                ...
                return render(request, "non_register_mb/login.html")  # Redirect to success page
        else:
...
def forgot(request):
    if request.method == "POST":
        ...
        if not input_digits:
            ...
            return render(request, "non_register_mb/forgot.html", {"show_otp_frame": False})
        ...
        for user in users:
            ...
            if db_phone_digits.endswith(input_digits[-10:]):  # Compare last 10 digits
                ...
                print(messages.get_level(request))
                return render(request, "non_register_mb/forgot.html", {"show_otp_frame": True})
...
def test_resetpass(request):
    ...
    if not user_role or not user_id:
        ...
        return redirect('forgot_mb')  # Redirect to forgot password page if no user info in session
    ...
    if request.method == "POST":
        ...
        return render(request, "non_register_mb/login.html", {"user_role": user_role})
