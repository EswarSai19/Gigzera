...
function toggleOtherInput() {
  ...
  var otherDesignationContainer = document.getElementById("otherDesignationContainer");
...
// Script for certificate names
  ...
  function showDropdown() {
    dropdown.innerHTML = "";
    const filteredOptions = certificationOptions.filter(option => !selectedCertifications.includes(option));
  ...
  function handleBackspace(event) {
    if (event.key === "Backspace" || event.key === "Delete") {
      let currentValues = input.value.split(",").map(val => val.trim()).filter(val => val);
      
      // Check for removed items
      selectedCertifications.forEach(cert => {
        if (!currentValues.includes(cert)) {
          // Add back to available options
          if (!certificationOptions.includes(cert)) {
            certificationOptions.push(cert);
          }
        }
      });

      selectedCertifications = currentValues;
